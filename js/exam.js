...
class QuestionNavigation {
    ...
    static markForReview(questionIndex) {
        ...
        if (btn) {
            ...
            const reviewList = Utils.getLocalStorage('marked_for_review') || [];
            const isMarked = btn.classList.contains('marked-for-review');
            
            if (isMarked && !reviewList.includes(questionIndex)) {
                reviewList.push(questionIndex);
            } else if (!isMarked && reviewList.includes(questionIndex)) {
                reviewList.splice(reviewList.indexOf(questionIndex), 1);
            }
            
            Utils.setLocalStorage('marked_for_review', reviewList);
        }
    }

    static loadMarkedQuestions() {
        const reviewList = Utils.getLocalStorage('marked_for_review') || [];
        reviewList.forEach(questionIndex => {
            const btn = document.querySelector(`.question-nav-btn:nth-child(${questionIndex + 1})`);
            if (btn) {
